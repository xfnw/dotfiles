...
def create_left_prompt [] {
    ...
    let host_segment = $"($host_color)($host)"

    let path_color = (if (is-admin) { ansi red } else { ansi green })
    let separator_color = (if (is-admin) { ansi light_red } else { ansi light_green })
    let path_segment = $"($path_color)($dir)" | str replace --all (char path_sep) $"($separator_color)(char path_sep)($path_color)"

    [$host_segment, $path_segment] | str join " "
}
