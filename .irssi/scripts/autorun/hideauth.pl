...
theme_register([
  'bot_command', 'bot command {reason $0 %K****%n$1} sent to {channick_hilight $2}'
]);
...
settings_add_str('misc','hideauth_commands','op voice auth ident identify pass newpass rehash');
settings_add_bool('misc','hideauth_botsonly',0);
settings_add_str('misc','hideauth_bots','NickServ');
